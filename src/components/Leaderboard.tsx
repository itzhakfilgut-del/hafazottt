    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        ...
        snapshot.forEach((doc) => {
          ...
          if (data.status === 'approved' && (data.clicks || 0) > 0) {
      ...
      <div className="divide-y divide-slate-100">
        {users.map((user, index) => (
          <div key={user.uid} className="p-4 flex items-center justify-between hover:bg-slate-50 transition-colors">
            ...
            <div className="flex items-center gap-4">
              <div className="text-xl font-black text-blue-600 bg-blue-50 px-4 py-1 rounded-full">
                {user.clicks || 0}
              </div>
              {isAdmin && (
                <div className="flex items-center gap-1 border-r pr-4 border-slate-200">
                  <button onClick={() => handleUpdateClicks(user.uid, 1, user.clicks)} className="p-1.5 bg-green-50 text-green-600 rounded-lg hover:bg-green-100 transition-colors" title={texts.admin.actions.addClick}>
                    <Plus size={16} />
                  </button>
                  <button onClick={() => handleUpdateClicks(user.uid, -1, user.clicks)} className="p-1.5 bg-orange-50 text-orange-600 rounded-lg hover:bg-orange-100 transition-colors" title={texts.admin.actions.removeClick}>
                    <Minus size={16} />
                  </button>
